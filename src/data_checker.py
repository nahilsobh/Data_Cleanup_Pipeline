...
class Checker:
    def __init__(self, run_parameters):
        ...
        self.output_values     = pd.DataFrame(index=checks_values)
        self.output_idx_header = pd.DataFrame(index=checks_idx_header)
    ...
    def condition_check(self):
        ...
        """
        self.output_idx_header["index" ] = Checker.check_index_header( pd.DataFrame(self.dataframe.  index.values))
        self.output_idx_header["header"] = Checker.check_index_header( pd.DataFrame(self.dataframe.columns.values))
        self.output_values    ["value" ] = Checker.check_values      ( dataframe=self.dataframe)

        IOUtil.write_to_file(self.output_values    , "data_statics_values"      , self.run_parameters['results_directory'], "_ETL.tsv")
        IOUtil.write_to_file(self.output_idx_header, "data_statics_index_header", self.run_parameters['results_directory'], "_ETL.tsv")
    ...
    def check_values(dataframe):
        ...
        output.append(False if False in dataframe.applymap(lambda x: isinstance(x, (int, float)) and x >= 0).values else True)
        output.append(False if set(pd.unique(dataframe.values.ravel())) != set([0, 1]) else True)
